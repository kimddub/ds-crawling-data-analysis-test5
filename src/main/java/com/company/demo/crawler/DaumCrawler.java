...
public class DaumCrawler {
	...
	//DB site에서 꺼내올것
	String listUrl = "https://media.daum.net/breakingnews"; // + "/society/people?" 
	String dateUrl = "&regDate=";
	String categoryUrl1 = "/";
	String categoryUrl2 = "/";
	String timeFormat = "yyyy.MM.dd. HH:mm";
	String lastPageSelector = "#mArticle > div.box_etc > div > span > a:last-child";
	// 다음에는 미디어 딸 때 다른 텍스트 제거해줘야 함
    //listPage.select("#mArticle > div.box_etc > ul > li > div > strong > span > span.txt_bar, #mArticle > div.box_etc > ul > li > div > strong > span > span.info_time").remove();
	String mediaSelector = "#mArticle > div.box_etc > ul > li > div > strong > span";
	// "언론사" 텍스트 처리도 해줘야 함
	String detailPageSelector = "#mArticle > div.box_etc > ul > li > div > strong > a";
	String dateSelector = "#cSub > div > span > span.txt_info:last-of-type";
	String bodySelector = "#harmonyContainer > section"; //
	String idSelectorInUrl = "/v/";
	...
	// 아직 기사 중복 해결은 안함 (리셋후 넣거나, 이 전 날짜의 기사 읽어오는 로직이 없는 상태이므로)
		...
		public List<Article> crawling(String sectionId, String subSectionId, Date limitDateBeforeSet) {
			...
			*/
	        ...
	        while(continueToCrawling) {
	        	...
	        	System.out.println("crawling page: " + crawlingPage); //크롤링 하는 페이지 넘김 확인
		...
		private Document getListPage(String crawlingDate, int crawlingPage) {
			...
			System.out.println(listUrl+categoryUrl+pageUrl+dateUrl);
		...
		private int getLastPageNum(String crawlingDate, int currentFirstPage) {
			...
			int lastPageNum = currentFirstPage;
			...
			for (Element pageNum:lastPageNums) {
				...
				if (pageNumStr.length() == 0) {
					// 숫자나 '다음' 텍스트가 안들어온건
					// 현재 1,11 .. 페이지가 활성화되있어서 구조가달라서 못가져왔을수 있다
					// 그게 아니면 현재 첫번째 페이지 제외하고는 끝 페이지나 다음이 넘어와야 한다
					return lastPageNum; 
					
				} else if (pageNumStr.equals("다음")) {
			...
			for (Map<String,String> articleInfo:articleInfoList) {
				...
				if (dateStr.length() != 0) {
					...
					try {
						...
						// Daum date 포맷 불필요 문자 제거
						// 다음 - 날짜형식 앞에 텍스트 포함된거 처리해야 함
						if (dateStr.contains("최종")) {
							
							dateStr = dateStr.split("최종")[1].trim();
							
						} 
						
						if (dateStr.contains("입력")) {

							dateStr = dateStr.split("입력")[1].trim();
						} 
					...
					System.out.println("date 를 크롤링 할 수 없음" + dateStr + "- path: " + detailPageUrl);
					date = null;
				}
...
//				article.setTitle(title);
				...
				if (article.getBody().length()==0 || article.getRegDate()==null) {
					System.out.println("crawling failed - path: " + detailPageUrl);
					System.out.println("\t body: " + article.getBody().length() + "\t regDate: " + article.getRegDate().toString());
		...
		private List<Map<String,String>> getArticleInfoListFromOnePage(Document listPage) {
			...
			for (Element media:mediaListHTML) {
				...
				// 다음 - "언론사" 텍스트 추출도 해줘야 함				
				mediaList.add(media.text().trim().replaceAll("\\\"", ""));
